...
func (h NotificationHandler) GetNotificationObject(c *fiber.Ctx) error {
	...
	sort.Slice(notiObject, func(i, j int) bool {
		return notiObject[i].CreatedAt.After(notiObject[j].CreatedAt)
	})
...
func (h NotificationHandler) SetNotificationRead(c *fiber.Ctx) error {
	body := new(dto.SetNotificationReadBody)
	if err := c.BodyParser(body); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(response.ErrorResponse{
			Success: false,
			Error:   err.Error(),
	...
	if err := h.notificationService.ReadNotificationObject(*body.NotiID); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(response.ErrorResponse{
			Success: false,
			Error:   err.Error(),
		})
...
func (h NotificationHandler) GetNotificationObjectByNotiID(c *fiber.Ctx) error {
	body := new(dto.GetNotificationObjectByNotiIDBody)
	if err := c.BodyParser(body); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(response.ErrorResponse{
			Success: false,
			Error:   err.Error(),
	...
	notiObject, err := h.notificationService.GetNotificationObjectByNotiID(*body.NotiID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(response.ErrorResponse{
			Success: false,
			Error:   err.Error(),
